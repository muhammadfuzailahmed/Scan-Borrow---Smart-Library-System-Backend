...
const borrowBook = async (req, res) => {
  ...
  const numberOfBorrowedBooksResult = await sql.query`select count(*) AS totalBorrowedBooks from transaction_records where userId = ${userId} and book_status = 'ISSUED'`

  const numberOfBorrowedBooks = numberOfBorrowedBooksResult.recordset[0].totalBorrowedBooks;
  ...
  const bookCopyIdResult = await sql.query`select bookCopyId from book_copies where QRcode = ${QRcode}`
  ...
  const updateIsIssued = await sql.query`update book_copies set isIssued = 1 where bookCopyId = ${bookCopyId}`

  const borrowedBookData = await sql.query`select * from transaction_records where userId = ${userId} and bookCopyId = ${bookCopyId}`
  ...
  const getBorrowedBookCopyCodeResult = await sql.query`select copyCode from book_copies where bookCopyId = ${bookCopyId}`
  ...
  const getBorrowedBookCopyCode = getBorrowedBookCopyCodeResult.recordset[0].copyCode;

     const bookNameResult = await sql.query`select bookName from books where bookId = (select bookId from book_copies where bookCopyId = ${bookCopyId})`
...
const getBorrowedBooksByCurrentUser = async (req, res) => {
  ...
  const getBooksQueryResult = await sql.query`select b.bookName, bc.copyCode, tr.transactionCode, tr.issueDate, tr.dueDate, tr.book_status AS status from transaction_records tr JOIN book_copies bc ON tr.bookCopyId = bc.bookCopyId JOIN books b ON bc.bookId = b.bookId where tr.book_status = 'ISSUED' and tr.userId = ${userId}`
...
const getCurrentUserBorrowingHistory = async (req, res) => {
  ...
  const getBooksQueryResult = await sql.query`select b.bookName, bc.copyCode, tr.transactionCode, tr.issueDate, tr.dueDate, tr.book_status AS status from transaction_records tr JOIN book_copies bc ON tr.bookCopyId = bc.bookCopyId JOIN books b ON bc.bookId = b.bookId where tr.userId = ${userId}`
...
export {getStudentDashboardData, borrowBook, getBorrowedBooksByCurrentUser, getCurrentUserBorrowingHistory}